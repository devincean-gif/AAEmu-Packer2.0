...
public partial class MainForm : Form
{
    ...
    private void LoadCustomReaders(string userFolder)
    {
        ...
        // Always ensure the AAFree 5.0 (ZERO) reader is available (baked in, no external file needed)
        var hasAAFree = AAPak.ReaderPool.Any(r => r.HeaderBytes is { Length: 4 } &&
        ...
        if (!hasAAFree)
        {
            var aafree = new AAPakFileFormatReader(true)
            {
                ...
                {
                    // 8 leading zero bytes before the name => Dummy2 moved to front
    ...
    private void MainForm_Load(object sender, EventArgs e)
    {
        ...
        MMVersion.Text = v + "  ·  ZeromusXYZ & Devincean";

        // Initialize Pak Readers
        AAPak.ReaderPool.Clear();

        // Portable: load readers.json from the app's own folder so it travels with the exe
